...
# def main(group_index, index, split_num, tt_list, name_list, seed_list):
  ...
  J_list = pickle.load(open("data_temp/group_5_val_{}.p".format(try_num), "rb"))['J_list']
  name_list_full = pickle.load(open("data_temp/group_5_val_{}.p".format(try_num), "rb"))['name_list']

  n = math.ceil(len(J_list)/ 30)

  tt_list = J_list[n * index:n * (index+1)]
  name_list = name_list_full[n * index:n * (index+1)]
  seed_list = [i+100*try_num for i in range(100)][n * index:n * (index+1)]

  print("total length", len(J_list), len(tt_list))
  del J_list, name_list_full

  for tt, seed, name in zip(tt_list, seed_list, name_list):
      file_check1 = os.path.join(save_dir, 'val', 'pass', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
      file_check2 = os.path.join(save_dir, 'val', 'fail', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        name_list = []
        new_pickle = {}
        for i in os.listdir('data_temp/group_5_{}/val/fail'.format(try_num)):
            data_path = os.path.join('data_temp/group_5_{}/val/fail'.format(try_num), i)
        ...
        count_pass = len(os.listdir('data_temp/group_5_{}/val/pass'.format(try_num)))
        count_fail = len(os.listdir('data_temp/group_5_{}/val/fail'.format(try_num)))
        cum_count_pass += count_pass

        try_num += 1
        with open("data_temp/group_5_val_{}.p".format(try_num), "wb") as f:
