...
def main(begin , end):
  ...
  num_nodes_I = 9
  num_graphs_train = 9
  save_dir = '/home/ubuntu/pycharm_project_home/data_temp/V=9/all_0.6'

  topology_list = [
    'bipartite', 'trigrid', 'grid', 'binarytree'
  ]
  ...
  for tt in topology_list:
    ...
    for ii in range(begin, end):
      sample_id = ii#
      seed_train = int(str(3333) + str(sample_id))#
      topology = NetworkTopology(num_nodes=num_nodes_I, seed=seed_train)#
      npr = np.random.RandomState(seed=seed_train)#
      graph = {}
      ...
      file_name = os.path.join(save_dir, 'test', 'graph_{}_nn{}_{:07d}.p'.format(tt, num_nodes_I, sample_id))
