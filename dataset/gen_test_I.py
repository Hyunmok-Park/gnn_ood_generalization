...
def main(begin, end):
  ...
  """
  #seed_test = int(str(4444)+str(sample_id))
  std_J_A = 0.9
  std_b_A = 0.25
  num_nodes_I = 16
  num_graphs_test = 10

  save_dir = '../data_temp_server/V=16/mycode/data_temp/train_5'
  ...
  for i in topology_list:
      try:
        mkdir(os.path.join(save_dir, i, "test"))  #!!
      except OSError:
        pass
  ...
  for tt, jj in zip(topology_list, file_list):
    ...
    for ii in range(begin, end):
      ...
      file_name = os.path.join(save_dir,tt,jj, 'graph_{}_nn{}_{:07d}.p'.format(tt, num_nodes_I, sample_id))
      with open(file_name, 'wb') as f:
        pickle.dump(graph, f)
        del graph


# python -m dataset.gen_test_I
