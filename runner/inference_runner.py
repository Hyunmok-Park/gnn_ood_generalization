...
__all__ = ['NeuralInferenceRunner', 'NeuralInferenceRunner_Meta', 'NeuralInferenceRunner_Meta2', 'AlgorithmicInferenceRunner_bp', 'AlgorithmicInferenceRunner', 'NeuralInferenceRunner_MT']
...
class NeuralInferenceRunner_Meta(object):
  ...
  def train(self):
    ...
    def _update_frac_worse_accepts(old_loss, new_loss, upt_factor, accept, SA_running_factor, SA_running_acc_rate):
    ...
    for step in tqdm(range(500), desc="META TEST"):
      for data in tqdm(test_loader):
        ...
        with torch.no_grad():
          ...
          log_prob, loss, loss_batch = model(data['edge_attr'], data['x'], data['edge_index'], data['idx_msg_edge'], target=data['y'], node_idx=node_idx, node_idx_inv=node_idx_inv)
...
class NeuralInferenceRunner_Meta2(object):
  def __init__(self, config, config2):
    ...
    self.initial_acc = 0
    self.temp_slope_opt_steps = self.train_conf.max_epoch
    self.temp_update = self.test_conf.temp
