...
# def main(group_index, index, split_num, tt_list, name_list, seed_list):
  ...
  save_dir = 'data_temp/group_5_{}'.format(try_num)
  J_list = pickle.load(open("data_temp/group_5_train_{}.p".format(try_num), "rb"))['J_list']
  name_list_full = pickle.load(open("data_temp/group_5_train_{}.p".format(try_num), "rb"))['name_list']
  ...
  for tt, seed, name in zip(tt_list, seed_list, name_list):
      file_check1 = os.path.join(save_dir, 'train', 'pass', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
      file_check2 = os.path.join(save_dir, 'train', 'fail', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
      if not os.path.exists(file_check1) and not os.path.exists(file_check2):
          seed_train = int(str(1111) + str(seed))
          ...
          try:
              ...
              mkdir(os.path.join(save_dir, 'train', 'pass'))
              mkdir(os.path.join(save_dir, 'train', 'fail'))
          except OSError:
              pass
          ...
          if diff < 0.02:
              file_name = os.path.join(save_dir, 'train', 'pass', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
          else:
              file_name = os.path.join(save_dir, 'train', 'fail', 'graph_{}_nn{}_{:07d}.p'.format(name, num_nodes_I, seed))
...
if __name__ == '__main__':
    ...
    while True:
        ...
        name_list = []
        new_pickle = {}
        for i in os.listdir('data_temp/group_5_{}/train/fail'.format(try_num)):
            data_path = os.path.join('data_temp/group_5_{}/train/fail'.format(try_num), i)
        ...
        count_pass = len(os.listdir('data_temp/group_5_{}/train/pass'.format(try_num)))
        count_fail = len(os.listdir('data_temp/group_5_{}/train/fail'.format(try_num)))
        cum_count_pass += count_pass

        try_num += 1
        with open("data_temp/group_5_train_{}.p".format(try_num), "wb") as f:
