...
def data_to_gpu(*input_data):
  ...
  for dd in input_data:
    if type(dd).__name__ == 'Tensor' or type(dd).__name__ == 'SparseTensor':
      return_data += [dd.cuda()]
...
def load_model(model, file_name, optimizer=None, train_pretext=False):
  # model_snapshot = torch.load(file_name)
  model_snapshot = torch.load(file_name, map_location='cpu')
  try:
    model.load_state_dict(model_snapshot["model"])
  except:
    model.module.load_state_dict(model_snapshot["model"])

  # if optimizer is not None and not train_pretext:
